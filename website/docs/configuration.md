...
sidebar_position: 2
title: Configuration
description: ChativaSettings and ThemeConfig — the two configuration objects, both JSON-Schema-validated.
---
...
|---|---|---|---|
| `ChativaSettings` | `window.chativaSettings` (before mount) | [`schemas/chativa-settings.schema.json`](https://github.com/AimTune/chativa/blob/main/schemas/chativa-settings.schema.json) | `application/ChativaSettings.ts` |
| `ThemeConfig` | `chatStore.getState().setTheme(...)` (any time) | [`schemas/theme.schema.json`](https://github.com/AimTune/chativa/blob/main/schemas/theme.schema.json) | `domain/value-objects/Theme.ts` |

`ChativaSettings.theme` is a deep partial of `ThemeConfig` — anything you pass is merged over `DEFAULT_THEME`.

> **Schema drift guard.** The `ThemeConfig` ↔ schema contract is enforced by `schema-drift.test.ts`. Any field added to the TypeScript type without a matching schema update fails CI. See [schemas/README.md](https://github.com/AimTune/chativa/blob/main/schemas/README.md) and [AGENTS.md → Schema sync](https://github.com/AimTune/chativa/blob/main/AGENTS.md#schema-sync-rule).
...
Full reference (all fields, defaults, enums, and constraints) lives in the schema: **[`schemas/theme.schema.json`](https://github.com/AimTune/chativa/blob/main/schemas/theme.schema.json)**. Highlights:
...
| `windowMode` | `popup \| side-panel \| fullscreen \| inline` | `popup` | Presentation style. See [theming → Window modes](./theming.md#window-modes). |
...
| `enableMultiConversation` | `boolean` | `false` | Conversation list view. Requires connector hooks. See [multi-conversation](./multi-conversation.md). |
