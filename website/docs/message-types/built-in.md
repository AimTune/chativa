...
Bot-authored `buttons` and `quick-reply` bubbles show the bot avatar, honouring the theme's `avatar.bot` URL and `showBot` flag — see [Theming → Avatars](../theming.md#avatars).
