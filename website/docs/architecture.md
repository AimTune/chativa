...
sidebar_position: 1
title: Architecture
description: Layered view of the Chativa monorepo, request flow, and the dependency rules that keep it clean.
---
...
Inner layers never import from outer layers. If you haven't read [Concepts](./concepts.md), start there — this page is the layered reference.
...
- The contract drift between `domain/` types and the JSON Schemas in `schemas/` is automatically guarded — see [schema sync on GitHub](https://github.com/AimTune/chativa/blob/main/schemas/README.md).
